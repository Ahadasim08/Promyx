...
- Final whole-branch review (sonnet) found one Important issue: `investigate()` had no error handling around the live Groq call, so a failure mid-batch would crash before any row was written, silently discarding the whole run's work (DDL auto-commit had already dropped/recreated `agent_reviews`). Fixed: wrapped the API call in try/except with a safe "open" fallback, and switched from one batched `executemany` at the end to per-row insert+commit, so partial progress survives a later failure.
...
3. Checking: accuracy = 18/19 = 0.95, false-accusation rate = 0/11 = 0.00 — `src/grade_checking.py`. Was 15/19 for a while because 3 rows (P05/P06/P08) disagreed with the answer key after their tickets were manually moved to Done in Jira during earlier verification testing. Resolved 2026-07-02 by updating `answer_key.csv`'s ground truth for those 3 rows to match the resulting reality (see `data/answer_key_notes.md` "Manually altered rows" section) — a deliberate choice, not a code fix; `check.py`'s decision logic never changed. Caveat: those 3 rows no longer have a frozen baseline, so further live Jira drift on them will again silently desync the answer key. Remaining miss (P10) is the pre-existing null-deadline gap, unrelated to this change — see metric 4.
4. Hard-case agent (Step 5): 1/19 promises flagged as hard cases (P10, Maria's "I'll take care of that thing with the checkout tests soon" — null deadline + vague wording) — `src/grade_agent.py`. Agent suggested_verdict accuracy on that hard case: 1/1 = 1.00 (agent correctly inferred "soon" implied a deadline before end of June, saw the ticket was still "In Progress" past that inferred deadline, and suggested "broken", matching ground truth). Earlier in the session (before a final-review fix) the same prompt hedged to "open" (0/1 = 0.00) — LLM output isn't fully deterministic even at temperature=0 for this kind of open-ended reasoning, so treat this single-hard-case number as illustrative of the mechanism working, not as a guarantee of accuracy at scale. Worth expanding the hard-case set (more transcripts) if Step 5 is revisited, to get a statistically meaningful accuracy number.
